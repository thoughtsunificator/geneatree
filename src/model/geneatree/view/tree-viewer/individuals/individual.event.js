...
class IndividualEventListener extends EventListener {
    ...
    remove() {
        if(this.properties.tree.selectedIndividual === individual) {
            this.properties.tree.individuals.filter(individual_ => individual_ !== individual).forEach(individual_ => individual_.emit("nodeShow"))
        }
        this.root.remove()
    }
