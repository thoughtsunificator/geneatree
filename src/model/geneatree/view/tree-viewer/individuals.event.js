...
class IndividualsEventListener extends EventListener {
	...
	select(data) {
		...
		if(this.properties.tree.selectedIndividual !== null) {
			this.properties.geneatree.individuals.emit("unselect", this.properties.tree.selectedIndividual)
		}
		this.properties.tree.selectedIndividual = data
		data.emit("select")
		this.properties.tree.individuals.filter(individual => individual !== data).forEach(individual => individual.emit("nodeHide"))
		this.properties.geneatree.explorer.emit("dragToggle", false)
	...
	*/
	...
	unselect(data) {
		...
		this.properties.geneatree.explorer.emit("dragToggle", true)
	...
	*/
	...
	update(data) {
		...
		data.individual.update(data.form)
