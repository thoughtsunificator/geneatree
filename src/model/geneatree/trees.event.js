...
class TreesEventListener extends EventListener {
	...
	add(data) {
		...
		if(data[2]) {
			for(const data_ of data[2]) {
				const relationshipIndividuals = []
				for(const relationshipIndividualData of data_.relationshipIndividuals) {
					const individual = tree.individuals.find(individual => individual.offlineId == relationshipIndividualData.offlineId)
					const relationshipIndividual = new RelationshipIndividual(individual, RelationshipIndividual.ROLES.SPOUSE)
					relationshipIndividuals.push(relationshipIndividual)
				}
				const relationship = tree.addRelationship(data_.type, relationshipIndividuals)
				if(data[0].type === "online") {
					relationship.networkId = data_._id
				} else if(data[0].type === "offline") {
					relationship.offlineId = data_.id
				}
			}
		}
